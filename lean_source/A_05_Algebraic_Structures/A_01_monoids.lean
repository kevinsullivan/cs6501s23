...
look at those. The extension itself adds the operation n_pow, which
is then defined recursively as applying a given operation *n* times. 
The proofs require base and recursive step cases. Let's ignore the
details for now. 

The major insight to gain from this definition is that to define a
monoid typeclass instance, we need to already have instances of the
(Lean's) semigroup and mul_one_class typeclasses over our monoid
elements. So Let's see what we need for those. We'll look first at
semigroup then at mul_one_class.
